...
export function LastSaleBanner({ salesType = "all" }: { salesType?: "office" | "support" | "renewal" | "all" }) {
  const [sale, setSale] = useState<LastSaleInfo | null>(null);
  const [elapsed, setElapsed] = useState("");

  useEffect(() => {
    const includeDeals = salesType === "all" || salesType === "office" || salesType === "support";
    const includeRenewals = salesType === "all" || salesType === "renewal";

    Promise.all([
      includeDeals ? fetchDeals() : Promise.resolve([]),
      includeRenewals ? fetchRenewals() : Promise.resolve([]),
    ])
      .then(([deals, renewals]) => {
        ...
        for (const d of deals) {
          ...
          const dealType = d.sales_type === "support" ? "support" : "office";
          if (salesType !== "all" && dealType !== salesType) continue;
