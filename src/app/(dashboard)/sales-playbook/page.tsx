...
const defaultData = {
  ...
  negotiation: {
    ...
    tactics: [
      { id: uid(), name: "المقايضة المشروطة", en: "Quid Pro Quo", body: "لا تمنح أي تنازل مجاناً. اربط الخصم بالتزام فوري: «أقدر أفعّل لك هذا الخصم الاستثنائي بشرط اعتماد الفاتورة والدفع خلال الساعات القادمة عشان يقبلها السيستم».", scenarios: [
        { id: uid(), text: "لو طلب تمديد مدة التجربة: «أقدر أمددها لك أسبوع زيادة، بشرط نثبّت سعر اليوم ونعتمد الاشتراك قبل ما تبدأ التجربة.»" },
      ] },
      { id: uid(), name: "التفاوض على القيمة لا السعر", en: "Value-Based", body: "عند شكوى السعر، لا تدافع عنه؛ أعد التذكير بحجم المشاكل التي يحلها المنتج والعائد والتوفير الذي سيحققه (ROI).", scenarios: [
        { id: uid(), text: "لو قارن بمنافس أرخص: «صح أرخص بالشهر، بس كم يكلّفك لو النظام وقف وقت الذروة؟ فرق السعر البسيط يرجع لك توفيراً في العمولة والأخطاء.»" },
      ] },
      { id: uid(), name: "الإرساء وثلاث باقات", en: "Anchoring", body: "قدّم ٣ باقات دائماً: باقة عالية جداً كمرساة نفسية، احترافية (وهي الهدف) بسعر عادل ومربح، وأساسية محدودة تجعل الاحترافية تبدو صفقة لا تُعوّض.", scenarios: [
        { id: uid(), text: "لو انصدم من سعر الباقة العليا: «هذي مصمّمة للسلاسل الكبيرة، وأنت الأنسب لك الاحترافية — نفس الجودة تقريباً بنص السعر.»" },
      ] },
      { id: uid(), name: "التعاطف التكتيكي والتسمية", en: "Tactical Empathy", body: "امتصّ هجوم العميل بالاعتراف بمخاوفه أولاً: «يبدو إنك حريص جداً على ميزانية الشركة وما تبي تخاطر بمبلغ بلا عائد واضح، صح؟».", scenarios: [
        { id: uid(), text: "لو قال «كلكم تقولون نفس الكلام»: «واضح إنك انلدغت قبل من وعود ما تحققت، صح؟ عشان كذا نبدأ بتجربة مقاسة بأرقام مطعمك لا بوعود.»" },
      ] },
      { id: uid(), name: "الرجوع لسلطة أعلى", en: "Higher Authority", body: "أظهر أنك في صف العميل تحارب قوانين الشركة أو السيستم لأجله: «أنا ودّي أخدمك، بس خلني أحاول أكلم الإدارة وأستخرج لك استثناء خاص».", scenarios: [
        { id: uid(), text: "لو ضغط أكثر على الخصم: «خذ راحتك، بس خلني أكون صريح — الاستثناء اللي طلعته لك من المدير سقفه اليوم، بعده يرجع السعر العادي.»" },
      ] },
      { id: uid(), name: "الأسئلة المعايرة", en: "Calibrated Questions", body: "انقل العميل من الهجوم إلى التفكير بأسئلة تبدأ بـ«كيف/ماذا»: «كيف تبغاني أقدّم لك نفس الجودة والدعم الفني اللحظي لو نزّلنا للسعر هذا؟».", scenarios: [
        { id: uid(), text: "لو قال «غالي مرّة»: «كيف وصلت إن السعر غالي؟ نقارنه بوش بالضبط عشان أوريك القيمة مقابل الرقم؟»" },
      ] },
    ],
    channels: [
    ...
    <div dir="rtl" style={{ fontFamily: "'Tajawal', sans-serif" }} className="min-h-screen bg-slate-100 text-slate-900">
      ...
      <SectionWrap id="sec-negotiation">
        ...
        <div className="mb-8">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {data.negotiation.tactics.map((t) => (
              <div key={t.id} className="flex flex-col rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                ...
                {/* سيناريوهات إضافية للتكتيك */}
                <div className="mt-3 border-t border-slate-100 pt-3">
                  <div className="mb-2 text-[11px] font-bold text-slate-400">سيناريوهات إضافية</div>
                  <div className="space-y-2">
                    {(t.scenarios || []).map((s, si) => (
                      <div key={s.id} className="flex items-start gap-1 rounded-lg p-2" style={{ background: "#f8fafc" }}>
                        <span className="mt-2 grid h-5 w-5 shrink-0 place-items-center rounded-md text-[10px] font-extrabold text-white" style={{ background: ACCENT }}>{si + 1}</span>
                        <AreaInput value={s.text} rows={2} onChange={(v) => update((d) => { d.negotiation.tactics.find((x) => x.id === t.id)!.scenarios!.find((y) => y.id === s.id)!.text = v; })} />
                        <IconBtn tone="red" title="حذف السيناريو" onClick={() => update((d) => { const tt = d.negotiation.tactics.find((x) => x.id === t.id)!; tt.scenarios = (tt.scenarios || []).filter((y) => y.id !== s.id); })}>✕</IconBtn>
                      </div>
                    ))}
                  </div>
                  <div className="mt-2">
                    <AddBtn onClick={() => update((d) => { const tt = d.negotiation.tactics.find((x) => x.id === t.id)!; tt.scenarios = [...(tt.scenarios || []), { id: uid(), text: "" }]; })}>إضافة سيناريو</AddBtn>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="mt-3">
            <AddBtn onClick={() => update((d) => { d.negotiation.tactics.push({ id: uid(), name: "تكتيك جديد", en: "", body: "", scenarios: [] }); })}>إضافة تكتيك</AddBtn>
          </div>
        </div>
